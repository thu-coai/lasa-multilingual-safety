...
sbatch_args=(-N 1 -c "${CPUS}" --output "${REPO_ROOT}/logs/train-classifier-%j.log" --error "${REPO_ROOT}/logs/train-classifier-%j.error")
if [[ -n "${PARTITION}" ]]; then
...
"${SBATCH_BIN}" "${sbatch_args[@]}" --wrap "cd '${REPO_ROOT}' && bash scripts/run_train_safety_classifier.sh"
