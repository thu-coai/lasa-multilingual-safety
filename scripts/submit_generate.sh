...
sbatch_args=(-N 1 -c "${CPUS}" --output "${REPO_ROOT}/logs/generate-%j.log" --error "${REPO_ROOT}/logs/generate-%j.error")
if [[ -n "${PARTITION}" ]]; then
...
"${SBATCH_BIN}" "${sbatch_args[@]}" --wrap "cd '${REPO_ROOT}' && bash scripts/run_generate.sh"
